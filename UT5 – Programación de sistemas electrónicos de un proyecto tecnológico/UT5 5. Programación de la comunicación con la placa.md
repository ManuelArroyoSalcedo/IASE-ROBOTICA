...
⚠️ Nota importante  
La función `analogWrite()` **no puede utilizarse en cualquier pin**.

Solo funciona en **determinados pines digitales que admiten PWM**, identificados en la placa con el símbolo `~`.

Los **pines analógicos (`A0`, `A1`, …)** se utilizan únicamente para **leer señales**, no para regular salidas.
